...
| 任务 ID | 任务描述 | 分配角色 | 预计小时 | 前置依赖 | 进度 |
| --- | --- | --- | --- | --- | --- |
| `T-0.11` | PostgreSQL 搭建：调研并落地 Docker（建议 docker compose）；接入 JDBC（Spring Data JDBC + PostgreSQL Driver + Flyway）；在 README 写清启动、连接、验证步骤 | 后端开发 × 1 | 4 | 无 | ⬜ |
| `T-0.12` | 基于现有 ER 编写 PostgreSQL schema SQL（含 `DROP TABLE IF EXISTS` + `CREATE TABLE`）；本地执行后完成全库创建 | 后端开发 × 1 | 4 | `T-0.11` | ⬜ |
| `T-0.13` | 按 ER 条目定义 DTO；为每个数据库实体建立 Service；新增/完善 DevRunner，对 DTO+Service+数据库连通进行启动验证 | 后端开发 × 1~2 | 6 | `T-0.12` | ⬜ |
|  |  | **冲刺总计** | **14 小时** |  | — |
...
flowchart LR
  ...
  t013[T_0_13_dto_service_devrunner]
...
| 文档 | 用途 |
| --- | --- |
| [SprintReleasePlan.md](../project_backlog/SprintReleasePlan.md) | 全量冲刺主计划 |
| [JavaBackendArchitecture.md](../project_backlog/JavaBackendArchitecture.md) | ER 与后端分层 |
| [ProductBacklog.md](../project_backlog/ProductBacklog.md) | 用户故事与优先级 |
