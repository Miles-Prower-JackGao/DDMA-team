...
**任务顺序（后端）：** `T-0.11`（PG + Docker + JDBC + README）→ `T-0.12`（ER 对齐 schema SQL 建库）→ `T-0.13`（DTO + Service + DevRunner 验证）。


| 任务 ID   | 任务描述                                                                 | 分配角色         | 预计小时      | 进度 |
| ------- | -------------------------------------------------------------------- | ------------ | --------- | ---- |
| T-0.11  | PostgreSQL 搭建：调研并落地 Docker（建议 docker compose）；接入 JDBC（Spring Data JDBC + PostgreSQL Driver + Flyway）；README 写清启动与连通验证 | 后端开发 × 1     | 4         | ⬜ |
| T-0.12  | 按现有 ER 编写 PostgreSQL schema SQL（`DROP TABLE IF EXISTS` + `CREATE TABLE`）并完成全库创建 | 后端开发 × 1     | 4         | ⬜ |
| T-0.13  | 按 ER 条目定义 DTO；为每个数据库实体建立 Service；增加 DevRunner 验证启动执行正确 | 后端开发 × 1~2   | 6         | ⬜ |
|         |                                                                      | **冲刺总计**     | **14 小时** | — |
...
**依赖链（数据与契约）：** 先完成 `T-0.11 ~ T-0.13` 的数据库主线（库可用 + schema 可用 + 数据层校验）→ 冲刺 1 实现注册、登录、Hub CRUD；OpenAPI 在冲刺 1 第 1 天冻结 baseline。
...
| 冲刺  | 周   | 阶段    | 覆盖故事                                           | 累计完成           | 进度 |
| --- | --- | ----- | ---------------------------------------------- | -------------- | ---- |
| 0   | 1   | ░ 基础  | T-0.11~T-0.13：PG+Docker+JDBC、ER Schema、DTO/Service/DevRunner | 数据层就绪         | 🔄 |
...
| 成员 # | 角色               | 冲刺 0                                  | 冲刺 1-5 (P0)         | 冲刺 6-7 (P1)   | 冲刺 8-9 (P2/QA)   | 进度 |
| ---- | ---------------- | ------------------------------------- | ------------------- | ------------- | ---------------- | ---- |
...
| 9    | **后端开发 C**       | **T-0.13**（DTO + Service + DevRunner）     | 支付 (Stripe)、订单 API  | 促销码、评分、问题 API | Apple/Google Pay | ⬜ |
