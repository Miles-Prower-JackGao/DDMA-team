...
| F9 | 全链路联调与 Demo Smoke | @Yanjia Kan |
...
[收尾] 联调 & Demo Smoke ──→ F9: develop 集成 + 冒烟回归 + demo 脚本确认
```
...
1. `F1 @Chuhan`：已在做，优先完成关键路径入口。
2. `F8 @Sida Xue`：这周可以早点开始，独立任务先并行推进。
3. `F2 @Qiyuan Huang`：紧跟 `F1`，尽快打通订单创建。
4. `F3 @Hangyi Gan`：在 `F2` 可用后立刻接手推荐链路。
5. `F4 @Lei Feng`：每天稳定 2 小时，持续推进支付链路并等待上游联通。
6. `F5 @Yuyang Zhou`：重点使用周三下午和周五白天窗口完成真实追踪接入。
7. `F6 @Jiayi Gao`：适合放到周五到周末收尾。
8. `F7 @Yiyuan Miao`：纯前端独立项，放到周五及之后推进。
9. `F9 @Yanjia Kan`：最后负责 develop 分支集成、全链路冒烟和 Demo 前验收。

---

## 任务总览

| 任务 | 描述 | 负责人 | 估时 | 依赖 | 优先级 | 状态 |
|------|------|--------|------|------|--------|------|
| F1 | 地址验证全链路 | @Chuhan | 1.5 天 | 无（Day 1） | P0 | 🟢 已完成 |
| F2 | 订单创建全链路 | @Qiyuan Huang | 2 天 | F1 | P0 🔑 | 🟢 已完成 |
| F3 | 配送推荐全链路 | @Hangyi Gan | 2 天 | F2 | P0 🔑 | 🔴 未开始 |
| F4 | 结账支付全链路 | @Lei Feng | 2 天 | F2 + F3 | P0 🔑 | 🟢 已完成 |
| F5 | 实时追踪全链路 | @Yuyang Zhou | 2 天 | F4 | P0 🔑 | 🟢 已完成 |
| F6 | 订单历史全链路 | @Jiayi Gao | 1 天 | F2 | P0 | 🔴 未开始 |
| F7 | 站点地图（纯前端）| @Yiyuan Miao | 1.5 天 | 无（Day 1） | P1 | 🔴 未开始 |
| F8 | 用户 Profile 更新 | @Sida Xue | 1.5 天 | 无（Day 1） | P1 | 🟢 已完成 |
| F9 | 全链路联调与 Demo Smoke | @Yanjia Kan | 1 天 | F4 + F5 + F6 + F7 + F8 | P0 | 🔴 未开始 |
...
#### DoD
- [x] `POST /validate/address`：SF 市中心地址返回 `valid: true`
- [x] `POST /validate/address`：纽约地址返回 `valid: false`
- [x] 前端地址步骤：无效地址时下一步按钮不可点击
...
#### DoD
- [x] 已登录用户可创建订单，返回 orderId（UUID）
- [x] 添加包裹成功，返回 parcelId
- [x] 未登录请求返回 401
- [x] 前端提交后携带 orderId 跳转到推荐页
...
#### DoD
- [x] `POST /orders/{id}/pay` 返回 handoffPin（6位数字）
- [x] Order 状态变为 `IN_TRANSIT`，FleetVehicle 标记为不可用
- [x] Payment 记录状态为 `SUCCEEDED`
- [x] 前端确认页显示真实 PIN 码 + ETA
...
#### DoD
- [x] 每次 GET 请求位置向目的地移动一步
- [x] 到达目的地后 status 自动变为 `DELIVERED`
- [x] 前端每 3 秒更新地图标记位置
- [x] 送达后轮询停止，显示完成状态
...
#### DoD
- [x] `PUT /users/me` 更新 fullName 和 phone 并返回最新信息
- [x] 不允许修改 email（后端忽略 email 字段，前端显示为只读）
- [x] 前端 ProfilePage 从 stub 变为真实功能页
- [x] 保存成功显示提示

---

### F9 — 全链路联调与 Demo Smoke 🔴
**负责人：** @Yanjia Kan  
**估时：** 1 天  
**依赖：** F4 + F5 + F6 + F7 + F8
...
- 汇总 `develop` 上的关键功能分支，完成 Demo 前的端到端联调。
- 用统一账号和固定演示路径跑通 MVP，提前暴露集成问题。
- 输出一份可复现的 Demo 操作顺序和 blocker 清单，供周五前集中收尾。

#### 具体工作

- 拉齐最新 `develop`，验证 `F1` 到 `F8` 的主链路是否能串起来。
- 按以下路径执行 smoke：
  - 注册/登录
  - 地址验证
  - 创建订单 + 添加包裹
...
- 记录每个阻塞点的：
  ...
  - 期望结果
  - 实际结果
- 输出 `Demo checklist` 和一版简短演示脚本。

#### DoD
- [ ] `develop` 分支上可完整跑通一遍 MVP 主链路
- [ ] 独立功能 `F7/F8` 已在联调环境可见
- [ ] 有一份 Demo 前可直接执行的 smoke checklist
- [ ] 所有 blocker 已明确归属到具体 owner
...
main
└── develop
    ├── feature/fs-f1-address-validation        (@Chuhan)
    ...
    └── feature/fs-f9-demo-smoke                (@Yanjia Kan)
```

- 每条分支独立开发，完成后 PR 合入 `develop`
- 关键路径任务（F2→F3→F4→F5）合入 `develop` 后由 `F9` 统一集成
...
### 联调收尾
- [ ] `F9` 产出 Demo checklist 和 blocker 清单
- [ ] Demo 演示账号、路径和兜底方案已确认
